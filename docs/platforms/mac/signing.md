...
- Preflights real identities by signing a temporary `/bin/echo` copy before modifying the app bundle. If macOS can see the certificate but cannot use its private key, signing fails early with keychain repair guidance.
...
Seeing a certificate in `security find-identity` does not prove that `codesign` can use its private key. If the preflight reports `errSecInternalComponent`, unlock the login keychain and grant Apple signing tools access, then rerun the packager:
...
If you import a `.p12` file, keep it paired with its export password. `SecKeychainItemImport: MAC verification failed during PKCS12 import` means the password does not match that file.
