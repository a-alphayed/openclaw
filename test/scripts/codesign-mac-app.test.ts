...
import { chmod, link, mkdir, readdir, rm, symlink, writeFile } from "node:fs/promises";
...
describe("codesign-mac-app temp file hygiene", () => {
  ...
  macIt.concurrent(
    "fails before bundle mutation when the signing private key is inaccessible",
    ({ mac, expect }) =>
      mac.lifetime.run(async () => {
        const tempRoot = mac.createTempDir("openclaw-codesign-preflight-");
        const app = path.join(tempRoot, "Fake.app");
        const binDir = path.join(tempRoot, "bin");
        await mkdir(path.join(app, "Contents", "MacOS"), { recursive: true });
        await mkdir(binDir);
        ...
        expect(tempEntries.filter((name) => name.startsWith("openclaw-entitlements"))).toEqual([]);
        expect(tempEntries.filter((name) => name.startsWith("openclaw-codesign-preflight"))).toEqual(
          [],
        );
      }),
  );
